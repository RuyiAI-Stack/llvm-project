...
// On RV64GC the C.NOP instruction is 2 byte long. The compiler may insert a
// J instruction (targeting the successor basic block), which accounts for
// another 4 bytes. An additional J instruction may appear (adding 4 more
// bytes) when the C.NOP is referenced elsewhere (ie. another branch).
// Depending on the compiler version the C.NOP may be omitted, leaving only
// the J instruction(s) before the label.
#define print_possible_return_addresses(addr)                                  \
  printf("%" PRIu64 ": current_address=%p or %p or %p or %p\n",               \
         ompt_get_thread_data()->value, ((char *)addr) - 2,                    \
         ((char *)addr) - 4, ((char *)addr) - 6,                              \
         ((char *)addr) - 10)
#else
